...
for z in range(20):
    for x in range(20):
        voxels[z*20+x+1] = [(x,0,z), None] # position, texture
...
def handle_messages(identifier: str):
    ...
    while True:
        ...
        if msg_json["object"] == "voxel":
            print("msg_decoded", msg_decoded)
            if msg_json["state"] == "create":
                voxels[int(msg_json["id"])] = [msg_json["position"], msg_json["texture"]]
            elif msg_json["state"] == "delete":
                print(voxels.keys())
                del voxels[int(msg_json["id"])]

            # Tell other players about blocks change
            for player_id in players:
                if player_id != identifier:
                    player_info = players[player_id]
                    player_conn: socket.socket = player_info["socket"]
        ...
        for player_id in players:
            if player_id != new_id:
                ...
                try:
                    ...
                    pass
        # Tell new player about existing voxels
        # for id_ in voxels:
        #     position, texture, color = voxels[id_]
        #     try:
        #         conn.send(json.dumps({
        #             "object": "voxel",
        #             "id": id_,
